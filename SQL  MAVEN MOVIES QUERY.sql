...
SELECT RATING,COUNT(*) as rating_cate_name
FROM FILM
GROUP BY RATING
ORDER BY rating_cate_name DESC;
...
SELECT EXTRACT(MONTH FROM RENTAL_DATE) AS MONYH_NUMBER,EXTRACT(YEAR FROM RENTAL_DATE) AS YEAR_NAME,COUNT(*) AS NUMBER_OF_RENTALS
FROM RENTAL 
...
SELECT *
FROM FILM 
WHERE SPECIAL_FEATURES LIKE "%BEHIND THE SCENCE%";
...
SELECT RATING,COUNT(*) AS COUNT_OF_MOVIES,MIN(LENGTH),ROUND(AVG (LENGTH),0) AS AVG_FILM_LENGTH,MAX(LENGTH), ROUND(AVG (RENTAL_DURATION),0) AS AVG_RENTAL_DURATION
FROM FILM
GROUP BY RATING
ORDER BY COUNT_OF_MOVIES DESC; 
...
SELECT F.FILM_ID,F.TITLE,COUNT(*) AS NUMBER_OF_ACTOR
